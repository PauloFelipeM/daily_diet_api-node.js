...
describe('Meals routes', () => {
  ...
  it('should not be able to access meals without a session', async () => {
    await request(app.server).get('/meals').expect(401)
  })

  it('should not be able to access meals from another user', async () => {
    ...
    await request(app.server)
      .post('/meals')
      .set('Cookie', user1Response.get('Set-Cookie')!)
      .send({
        name: 'Breakfast',
        description: "It's a breakfast",
        isOnDiet: true,
    ...
    const mealsFromUser1 = await request(app.server)
      .get('/meals')
      .set('Cookie', user1Response.get('Set-Cookie')!)
      .expect(200)

    const mealId = mealsFromUser1.body.meals[0].id

    const user2Response = await request(app.server)
      .post('/users')
      .send({ name: 'Jane Doe', email: 'janedoe@gmail.com' })
      .expect(201)

    await request(app.server)
      .get(`/meals/${mealId}`)
      .set('Cookie', user2Response.get('Set-Cookie')!)
      .expect(404)
  })
})
